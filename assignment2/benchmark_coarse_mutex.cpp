...
int main(int argc, char* argv[]) {
	...
	/* example use of benchmarking */
	{
		...
		benchmark(threadcnt, u8"thread_safe_coarse_mutex read", [&l1](int random){
			read(l1, random);
		});
		benchmark(threadcnt, u8"thread_safe_coarse_mutex update", [&l1](int random){
			update(l1, random);
		});
	}
	{
		...
		benchmark(threadcnt, u8"thread_safe_coarse_mutex mixed", [&l1](int random){
			mixed(l1, random);
		});
	}
	return EXIT_SUCCESS;
